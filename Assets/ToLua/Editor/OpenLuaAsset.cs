...
public class OpenLuaAsset : Editor
{
    ...
    private static string GetErrorViewRowCount(string condition, ref int line)
    {
        ...
        if (index >= 0)
        {
            ...
            int fileNameStart = condition.LastIndexOf(": ", index, StringComparison.Ordinal);
            if (fileNameStart <= 0) return null;

            int lineStart = fileNameEnd;
            int lineEnd = condition.IndexOf(": ", lineStart, StringComparison.Ordinal);
        ...
        if (index >= 0)
        {
            ...
            {*/
                ...
                if (fileNameStart <= 0) return null;
                int fileNameEnd = index + 4;
                int lineEnd = condition.IndexOf(":", fileNameEnd + 1, StringComparison.Ordinal);
                if (lineEnd <= 0) return null;
                string _line = condition.Substring(fileNameEnd + 1, lineEnd - fileNameEnd - 1);
                if (!Int32.TryParse(_line, out line)) return null;
        ...
        {
            int lineEnd = condition.LastIndexOf(": ", StringComparison.Ordinal);
            if (lineEnd <= 0) return null;
            int fileNameEnd = condition.LastIndexOf(":", lineEnd - 1, StringComparison.Ordinal);
            if (fileNameEnd <= 0) return null;
            int fileNameStart = condition.LastIndexOf(":", fileNameEnd - 1, StringComparison.Ordinal);
            if (fileNameStart <= 0) return null;                        
            string _line = condition.Substring(fileNameEnd + 1, lineEnd - fileNameEnd - 1);
            if (!Int32.TryParse(_line, out line)) return null;
