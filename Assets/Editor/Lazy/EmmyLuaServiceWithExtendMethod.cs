...
namespace EmmyLua
{
	...
	class EmmyLuaService
	{
		...
		private static void OnConnect(IAsyncResult ar)
		{
			...
			{
				//Debug.LogWarning("EmmyLuaServiceError:"+e.Message);
				TryLater();
			}
		}
				...
				foreach (var type in types)
				{
					...
					foreach (var extendMethod in extendsMethods)
					{
						...
						if (type2ExtendMethod.ContainsKey(thisType))
						{
							...
							var isss = type2ExtendMethod.TryGetValue(thisType,out methods);
                            if (isss)
                            {
			...
			if (!string.IsNullOrEmpty(fullName))
			{
				...
				var methods =
					(from mi in type.GetMethods(BindingFlags.Public | BindingFlags.Instance| BindingFlags.Static | BindingFlags.DeclaredOnly)
						...
						select mi).ToArray();
				if (!type2ExtendMethod.TryGetValue(type, out var extendMethods))
				{
